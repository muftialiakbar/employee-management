...
(function() {
    ...
    $(".todo-widget input[type='text']").keypress(function(event){ //Any key that was pressed in input is registered
        if(event.which === 13){ //every key has thier own id, enter key is number 13
            ...
            $(".todo-widget ul").append("<li class='listdata-group-item listdata-group-item-action'><span class='float-right'><i class='icon-close2 text-danger'></i></span>  " + todoText + "</li>"); //create a new LI and adds the text in the todoText variable into the UL
        }
    });
}());
